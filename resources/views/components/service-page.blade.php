...
<div class="bg-forest-deep py-16 sm:py-20 md:py-24" data-reveal>
    <div class="w-11/12 sm:w-9/12 md:w-8/12 mx-auto flex flex-col items-center gap-4 sm:gap-5 text-center">
        ...
        <p class="text-cream font-agency text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-snug sm:leading-tight">
            &ldquo;{{ $service['approach_statement'] }}&rdquo;
        </p>
    </div>
</div>
...
<div class="bg-mist py-20 sm:py-24 md:py-28">
    ...
    {{-- What's included --}}
    <div id="included" class="w-11/12 mx-auto 2xl:w-10/12 mt-16 sm:mt-20 md:mt-28" data-reveal>
        ...
        {{--
            Each deliverable gets a real explanation, not just a label — this
            is the section a visitor actually reads to learn what the
            service means in practice, so it's a stacked, divided list
            (title left, description right) rather than another grid of
            short pills or cards.
        --}}
        <div class="w-full flex flex-col divide-y divide-forest/10 border-t border-forest/10 mt-8 sm:mt-10 md:mt-12">
            @foreach ($service['deliverables'] as $item)
                <div class="grid grid-cols-1 md:grid-cols-4 gap-2 md:gap-8 py-6 sm:py-7 md:py-8">
                    <h4 class="font-agency font-bold text-forest text-lg sm:text-xl md:col-span-1">{{ $item['title'] }}</h4>
                    <p class="text-forest/70 text-sm sm:text-base md:col-span-3 leading-relaxed">{{ $item['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div id="work" class="w-11/12 mx-auto 2xl:w-10/12 mt-16 sm:mt-20 md:mt-28" data-reveal>
            ...
            <div class="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 mt-8 sm:mt-10 md:mt-12">
                @foreach ($caseStudies as $project)
                    <a href="{{ $project['href'] }}" class="group flex flex-col items-start gap-3 sm:gap-4">
                        <div
                            class="w-full overflow-hidden h-60 sm:h-65 md:h-70 relative rounded-2xl sm:rounded-3xl"
                        >
                            <img
        ...
        @else
            ...
            >
                <p class="text-forest/70 font-medium text-sm sm:text-base max-w-lg mx-auto">
                    We're still building out public case studies for this specific service. Take a look at our full
                    portfolio, or get in touch about your project.
                </p>
